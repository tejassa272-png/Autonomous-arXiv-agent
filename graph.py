...
def route_after_metadata(state: AgentState) -> str:
    """
    Graph-level branch: routes directly to parsing if the user provided an ID, 
    otherwise routes to Gemini to select the best paper from the topic search.
    """
    if state.get("error_message"):
        return "error"
...
def build_arxiv_agent():
    ...
    # Dynamic Semantic Router (Replaces the static edge)
